...
function windowStateKeeper() {
  ...
  try {
    ...
    log.warn("Could not calculate default screen size");
  }
...
function createWindow() {
  ...
  mainWindow.webContents.on("new-window" as any, function (e, externalurl) {
    e.preventDefault();
    shell.openExternal(externalurl as unknown as string);
  });
...
function sendStatusToWindow(progressObj) {
  ...
  mainWindow.setTitle(
    `Nault - ${autoUpdater.currentVersion} - Downloading Update: ${Math.round(progressObj.percent)} %`
  );
}
      ...
      submenu: [
        {
          ...
          click() {
            loadExternal("https://docs.nault.cc/");
          },
        },
        {
          label: "Reddit (r/nanocurrency)",
          click() {
            loadExternal("https://www.reddit.com/r/nanocurrency");
          },
        },
        {
          label: "Discord (#nault)",
          click() {
            loadExternal("https://discord.nanocenter.org/");
          },
        },
        { type: "separator" },
        {
          label: "View GitHub",
          click() {
            loadExternal("https://github.com/Nault/Nault");
          },
        },
        {
          label: "Submit a bug report",
          click() {
            loadExternal("https://github.com/Nault/Nault/issues/new");
          },
        },
        {
          label: "Release notes",
