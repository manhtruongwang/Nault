...
export class AccountsComponent implements OnInit {
  ...
  async createAccount() {
    ...
    if (
      this.isLedgerWallet &&
      this.ledger.ledger.status !== LedgerStatus.READY
    ) {
    ...
    if (!this.walletService.isConfigured())
      return this.notificationService.sendError(
        this.translocoService.translate("accounts.wallet-is-not-configured")
      );
    if (this.walletService.wallet.accounts.length >= 200)
      return this.notificationService.sendWarning(
        this.translocoService.translate(
          "accounts.you-can-only-track-up-to-x-accounts-at-a-time",
          { accounts: 200 }
        )
      );
    ...
    if (this.viewAdvanced && this.newAccountIndex != null) {
      ...
      if (index < 0)
        return this.notificationService.sendWarning(
          this.translocoService.translate(
            "accounts.invalid-account-index-must-be-positive-number"
          )
        );
      const existingAccount = this.walletService.wallet.accounts.find(
        (a) => a.index === index
      );
      if (existingAccount) {
        return this.notificationService.sendWarning(
          this.translocoService.translate(
            "accounts.the-account-at-this-index-is-already-loaded"
          )
        );
      }
      accountIndex = index;
    ...
    try {
      const newAccount =
        await this.walletService.addWalletAccount(accountIndex);
      this.notificationService.sendSuccess(
        this.translocoService.translate(
          "accounts.successfully-created-new-account",
      ...
      this.notificationService.sendError(
        this.translocoService.translate("accounts.unable-to-add-new-account", {
          error: err.message,
        })
      );
    }
  ...
  sortAccounts() {
    ...
    // }
    if (
      this.walletService.isLocked() ||
      !this.walletService.isConfigured() ||
      this.walletService.wallet.accounts.length <= 1
    )
      return;
    this.walletService.wallet.accounts =
      this.walletService.wallet.accounts.sort((a, b) => a.index - b.index);
  ...
  copied() {
    ...
    this.notificationService.sendSuccess(
      this.translocoService.translate(
        "general.successfully-copied-to-clipboard"
      ),
  ...
  async deleteAccount(account) {
    ...
    try {
      ...
      this.notificationService.sendSuccess(
        this.translocoService.translate(
          "accounts.successfully-removed-account",
          { account: account.id }
        )
      );
      ...
      this.notificationService.sendError(
        this.translocoService.translate("accounts.unable-to-delete-account", {
          error: err.message,
        })
      );
    }
  ...
  async showLedgerAddress(account) {
    ...
    this.notificationService.sendInfo(
      this.translocoService.translate(
        "accounts.confirming-account-address-on-ledger-device"
      ),
    ...
    try {
      ...
      this.notificationService.sendSuccess(
        this.translocoService.translate(
          "accounts.account-address-confirmed-on-ledger"
        )
      );
    } catch (err) {
      this.notificationService.sendError(
        this.translocoService.translate(
          "accounts.account-address-denied-if-it-is-wrong-do-not-use-the-wallet"
        )
      );
    }
