...
export class AddressBookComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async ngOnInit() {
    ...
    this.refreshSub = this.walletService.wallet.refresh$.subscribe(
      (shouldRefresh) => {
        if (shouldRefresh) {
          ...
          for (const entry of this.addressBookService.addressBook) {
            ...
            const walletAccount = this.walletService.wallet.accounts.find(
              (a) => a.id === entry.account
            );
            if (walletAccount) {
              // Subtract first so we can add back any updated amounts
              this.totalTrackedBalance = this.totalTrackedBalance.minus(
                this.accounts[entry.account].balance
              );
              this.totalTrackedBalanceRaw = this.totalTrackedBalanceRaw.minus(
                this.accounts[entry.account].balanceRaw
              );
              this.totalTrackedBalanceFiat =
                this.totalTrackedBalanceFiat -
                this.accounts[entry.account].balanceFiat;
              this.totalTrackedPending = this.totalTrackedPending.minus(
                this.accounts[entry.account].pending
              );

              this.accounts[entry.account].balance = walletAccount.balance;
              this.accounts[entry.account].pending = walletAccount.pending;
              this.accounts[entry.account].balanceFiat =
                walletAccount.balanceFiat;
              this.accounts[entry.account].balanceRaw =
                walletAccount.balanceRaw;

              this.totalTrackedBalance = this.totalTrackedBalance.plus(
                walletAccount.balance
              );
              this.totalTrackedBalanceRaw = this.totalTrackedBalanceRaw.plus(
                walletAccount.balanceRaw
              );
              this.totalTrackedBalanceFiat =
                this.totalTrackedBalanceFiat + walletAccount.balanceFiat;
              this.totalTrackedPending = this.totalTrackedPending.plus(
                this.accounts[entry.account].pending
              );
            }
          }
  ...
  ngAfterViewInit() {
    ...
    document
      .getElementById("address-book-sortable")
      .addEventListener("moved", (e) => {
        ...
        this.notificationService.sendSuccess(
          this.translocoService.translate(
            "address-book.updated-address-book-order"
          )
        );
      });
  }
  ...
  async updateTrackedBalances(refresh = false) {
    ...
    this.timeoutIdAllowingRefresh = setTimeout(
    ...
    const accountIDsWallet = this.walletService.wallet.accounts.map(
      (a) => a.id
    );
    ...
    if (this.appSettings.settings.minimumReceive) {
      const minAmount = this.util.nano.mnanoToRaw(
        this.appSettings.settings.minimumReceive
      );
      pending = await this.api.accountsPendingLimitSorted(
        accountIDs,
        minAmount.toString(10)
      );
    } else {
    ...
    for (const entry of this.addressBookService.addressBook) {
      ...
      if (walletAccount) {
        ...
        balanceAccount.balance = new BigNumber(
          apiAccounts.balances[entry.account].balance
        );
        balanceAccount.balanceFiat = this.util.nano
          .rawToMnano(balanceAccount.balance)
          .times(this.fiatPrice)
          .toNumber();
        balanceAccount.balanceRaw = new BigNumber(balanceAccount.balance).mod(
          this.nano
        );
      }
      this.totalTrackedBalance = this.totalTrackedBalance.plus(
      ...
      for (const block in pending.blocks) {
        ...
        if (pending.blocks[block]) {
          ...
          for (const hash in pending.blocks[block]) {
            ...
            accountPending = accountPending.plus(
  ...
  async saveNewAddress() {
    if (!this.newAddressAccount || !this.newAddressName) {
      return this.notificationService.sendError(
        this.translocoService.translate(
          "address-book.account-and-name-are-required"
        )
      );
    }
    ...
    if (regexp.test(this.newAddressName) === true) {
      return this.notificationService.sendError(
        this.translocoService.translate(
          "address-book.this-name-is-reserved-for-wallet-accounts-without-a-label"
        )
      );
    }
    ...
    if (this.newAddressName.startsWith("@") === true) {
      return this.notificationService.sendError(
        this.translocoService.translate(
          "address-book.this-name-is-reserved-for-decentralized-aliases"
        )
      );
    }

    // Remove spaces and convert to nano prefix
    this.newAddressAccount = this.newAddressAccount
      .replace(/ /g, "")
      .replace("xrb_", "nano_");

    // If the name has been changed, make sure no other entries are using that name
    if (
      this.newAddressName !== this.previousAddressName &&
      this.addressBookService.nameExists(this.newAddressName)
    ) {
      return this.notificationService.sendError(
        this.translocoService.translate(
          "address-book.this-name-is-already-in-use-please-use-a-unique-name"
        )
      );
    }
    ...
    if (!valid) {
      return this.notificationService.sendWarning(
        this.translocoService.translate(
          "address-book.account-id-is-not-a-valid-account"
        )
      );
    }

    // Store old setting
    const wasTransactionTracked =
      this.addressBookService.getTransactionTrackingById(
    ...
    try {
      ...
      this.notificationService.sendError(
        this.translocoService.translate("address-book.unable-to-save-entry", {
          message: err.message,
        })
      );
    }
  ...
  copied() {
    ...
    this.notificationService.sendSuccess(
      this.translocoService.translate(
        "address-book.account-address-copied-to-clipboard"
      ),
  ...
  async deleteAddress(account) {
    try {
      ...
      this.notificationService.sendSuccess(
        this.translocoService.translate(
          "address-book.successfully-deleted-address-book-entry"
        )
      );
      ...
      this.notificationService.sendError(
        this.translocoService.translate("address-book.unable-to-delete-entry", {
          message: err.message,
        })
      );
    }
  ...
  exportAddressBookToFile() {
    ...
    this.notificationService.sendSuccess(
      this.translocoService.translate(
        "address-book.address-book-export-downloaded"
      )
    );
  }
  ...
  importFromFile(files) {
    ...
    reader.onload = (event) => {
      ...
      try {
        ...
        this.notificationService.sendError(
          this.translocoService.translate(
            "address-book.unable-to-parse-import-data-make-sure-you-selected-the-right"
          )
        );
      }
    };
